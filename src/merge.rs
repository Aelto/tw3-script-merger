...
use crate::cli;
use crate::conflict::{print_json_conflict, prompt_conflicts_in_file};
use crate::resolver;
...
pub fn merge_mod(origin: &Path, output: &Path, modfolder: &Path) -> Result<(), Box<dyn Error>> {
  let content_folder = modfolder.join("content").join("scripts");
...
fn try_merge(
  origin_file_path: &Path, output_file_path: &Path, mod_path: &Path, modname: &str,
...
fn show_conflicts_to_user(
  output_file_path: &Path, content: &str, modname: &str,
) -> Result<(), Box<dyn Error>> {
